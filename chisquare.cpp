...
void chisquare(doub F[], doub LP[], doub CP[], doub& chisq, doub& chisq_I, int dof=12, int dof_I=7, int n_par=3)
{
  ...
  const bool trustLP87=true;//whether to fit for LP fraction at 87GHz. Its obs \
  ...
  for(int il=0;il<dof;il++) {
    ...
    chisq+=resid[il]*resid[il]/(dof-3); //calculate \chi^2/dof , 3 parameters are varied: inclination, rho_nor, C
    if (il<dof_I) chisq_I+=resid[il]*resid[il]/(dof_I-3); //calculate \chi^2/dof , 3 parameters are varied: inclination, rho_nor, C
