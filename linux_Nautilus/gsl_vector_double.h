...
_gsl_vector_view 
gsl_vector_view_array (doub *v, size_t n);

_gsl_vector_view 
gsl_vector_view_array_with_stride (doub *base,
                                         size_t stride,
                                         size_t n);

_gsl_vector_const_view 
gsl_vector_const_view_array (const doub *v, size_t n);

_gsl_vector_const_view 
gsl_vector_const_view_array_with_stride (const doub *base,
                                               size_t stride,
                                               size_t n);
...
void gsl_vector_set_all (gsl_vector * v, doub x);
...
doub gsl_vector_max (const gsl_vector * v);
doub gsl_vector_min (const gsl_vector * v);
void gsl_vector_minmax (const gsl_vector * v, doub * min_out, doub * max_out);
...
int gsl_vector_scale (gsl_vector * a, const doub x);
int gsl_vector_add_constant (gsl_vector * a, const doub x);
...
INLINE_DECL doub gsl_vector_get (const gsl_vector * v, const size_t i);
INLINE_DECL void gsl_vector_set (gsl_vector * v, const size_t i, doub x);
INLINE_DECL doub * gsl_vector_ptr (gsl_vector * v, const size_t i);
INLINE_DECL const doub * gsl_vector_const_ptr (const gsl_vector * v, const size_t i);
...
INLINE_FUN
void
gsl_vector_set (gsl_vector * v, const size_t i, doub x)
{
#if GSL_RANGE_CHECK
  if (GSL_RANGE_COND(i >= v->size))
    {
...
#endif
  return (const doub *) (v->data + i * v->stride);
}
#endif /* HAVE_INLINE */
