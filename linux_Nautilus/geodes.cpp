//depends on global variables: a - be very careful with global dependencies in parallel codes!!!
//int i;
doub rsq=y[4]*y[4], musq=y[5]*y[5], sinmu=sqrt(1.-musq),rhosq=rsq + asq*musq, rhogmin=rsq - asq*musq, rhosqsq=rhosq*rhosq, rasq=rsq+asq, dx=rasq-2.*y[4];
doub sinmusq=sinmu*sinmu, gg001=(rasq*rhogmin)/(dx*rhosqsq),gg002=(-2.*asq*y[4]*sinmu*y[5])/rhosqsq,
gg013=(a*sinmusq*(-3.*rsq*rsq+asq*asq*musq-asq*rsq*(1.+musq)))/(dx*rhosqsq),
gg023=(2.*asq*a*y[4]*sinmu*sinmusq*y[5])/rhosqsq,gg100=(dx*rhogmin)/(rhosqsq*rhosq),gg103=-((a*sinmusq*dx*rhogmin)/(rhosqsq*rhosq)),
gg111=(1.-y[4])/dx+y[4]/rhosq,gg112=-((asq*sinmu*y[5])/rhosq),gg122=-((y[4]*dx)/rhosq),
...
gg203=(2.*a*y[4]*rasq*sinmu*y[5])/(rhosqsq*rhosq),gg211=(asq*sinmu*y[5])/(dx*rhosq),gg212=y[4]/rhosq,
gg222=-((asq*sinmu*y[5])/rhosq),gg233=-((sinmu*y[5]*(2.*y[4]*rasq*rasq+dx*rhosqsq))/(rhosqsq*rhosq)),gg301=(a*rhogmin)/(dx*rhosqsq),
gg302=(-2.*a*y[4]*y[5])/(sinmu*rhosqsq),gg313=((-2.+y[4])*rsq*rsq+asq*rsq*(-1.+(-1.+2.*y[4])*musq)+asq*asq*(musq+(-1.+y[4])*musq*musq))/(dx*rhosqsq),
gg323=(y[5]*(2.*asq*y[4]*sinmusq+rhosqsq))/(sinmu*rhosqsq);//defining Christoffel symbols=connection coefficients
//printf(" %f %f\n",a, asq);
f[0] = y[6];
f[1] = y[7];
f[4] = y[2];
f[5] = y[3];//converting 2-nd order ODE into 1-st order ODE
