...
rk4_alloc (size_t dim)
{
  ...
  state->k = (doub *) malloc (dim * sizeof (doub));
  ...
  state->y_onestep = (doub *) malloc (dim * sizeof (doub));
...
static int
rk4_step (doub *y, const rk4_state_t *state,
	  const doub h, const doub t, const size_t dim,
	  const gsl_odeiv_system *sys)
{
  /* Makes a Runge-Kutta 4th order advance with step size h. */
  
  /* initial values of variables y. */
  const doub *y0 = state->y0;
  
  /* work space */
  doub *ytmp = state->ytmp;
           ...
           const gsl_odeiv_system * sys)
{
  ...
  doub *const k = state->k;
  doub *const k1 = state->k1;
  doub *const y0 = state->y0;
  doub *const y_onestep = state->y_onestep;
