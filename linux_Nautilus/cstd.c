...
std_control_init (void * vstate, 
                  doub eps_abs, doub eps_rel, doub a_y, doub a_dydt)
{
...
static int
std_control_hadjust(void * vstate, size_t dim, unsigned int ord, const doub y[], const doub yerr[], const doub yp[], doub * h)
{
  std_control_state_t *state = (std_control_state_t *) vstate;

  const doub eps_abs = state->eps_abs;
  const doub eps_rel = state->eps_rel;
  const doub a_y     = state->a_y;
  const doub a_dydt  = state->a_dydt;

  const doub S = 0.9;
  const doub h_old = *h;

//  doub rmax = DBL_MIN; //for float
doub rmax = 1e-32;
  size_t i;
  ...
  for(i=0; i<dim; i++) {
    ...
    const doub r  = fabs(yerr[i]) / fabs(D0);
  ...
  if(rmax > 1.1) {
    ...
    doub r =  S / pow(rmax, (doub)1.0/ord);
    ...
    *h = r * h_old;
    ...
    doub r = S / pow(rmax, (doub)1.0/(ord+(doub)1.0));
...
gsl_odeiv_control *
gsl_odeiv_control_standard_new(doub eps_abs, doub eps_rel,
                               doub a_y, doub a_dydt)
{
  gsl_odeiv_control * c = 
...
gsl_odeiv_control *
gsl_odeiv_control_y_new(doub eps_abs, doub eps_rel)
{
...
gsl_odeiv_control *
gsl_odeiv_control_yp_new(doub eps_abs, doub eps_rel)
{
