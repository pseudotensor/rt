...
gsl_odeiv_evolve_alloc (size_t dim)
{
  ...
  e->y0 = (doub *) malloc (dim * sizeof (doub));
  ...
  e->yerr = (doub *) malloc (dim * sizeof (doub));
  ...
  e->dydt_in = (doub *) malloc (dim * sizeof (doub));
  ...
  e->dydt_out = (doub *) malloc (dim * sizeof (doub));
...
gsl_odeiv_evolve_apply (gsl_odeiv_evolve * e,
                        ...
                        doub *t, doub t1, doub *h, doub y[])
{
  const doub t0 = *t;
  doub h0 = *h;
  int step_status;
  int final_step = 0;
  doub dt = t1 - t0;  /* remaining time, possibly less than h */
...
//          doub t_curr = GSL_COERCE_DBL(*t);
//          doub t_next = GSL_COERCE_DBL((*t) + h0);
          doub t_curr=(*t);
          doub t_next =(*t) + h0;
